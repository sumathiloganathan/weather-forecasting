...
public class SimpleAdjusterWithTime implements DataAdjuster{
	...
	public SimpleAdjusterWithTime(Set<DataPoint> data){
	...
	public Hashtable<MLDataPair,DataPoint[]> makeTraningData(SortedSet<DataPoint> data, int number) {
		Hashtable<MLDataPair,DataPoint[]> result = new Hashtable<MLDataPair,DataPoint[]>();
		...
		while (it.hasNext()){
			...
			if (pair!=null){
				i++;
				result.put(pair,new DataPoint[]{current,next});
			}
			current = next;
		}
	...
	public double[] asInput(DataPoint dp) {
		...
		ret[5] = (float)(dp.getMonth()+1)/13.0f;
		return ret;
	}
